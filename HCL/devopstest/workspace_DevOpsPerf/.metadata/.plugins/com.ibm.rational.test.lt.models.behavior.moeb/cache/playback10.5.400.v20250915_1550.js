...
/* Returns the value of the property given by id of the given element. 
   The check for input type inputtextfield is because , In the DDT we 
   have only content property and we have no property called content in 
   actual html. Hence it always returns blank incase of actual application 
   playback . The content  can be still placeholder or the actual value of it. 
   Hence Added a check on this. Since the issue was only with DDT i have added a 
   check for type. This type argument is only being passed if guided healing is enabled. 
   Otherwise it works as it is and calls getProperty(id.name); Refer TP-96939 for more detail*/
function /*Object*/ RMoTgetIdentifiedByValue(/*Object*/ elem, /*DeviceId*/ id, /*Object type*/type) {
	var ret = null;
		if (id && id.name) {
			var proxy = domainManager.getProxy(elem).proxyClass
			if(type=="html.uiinputtextfield" && id.name =="content" && proxy =="HtmlTextInputProxy"){
				var value = domainManager.getProxy(elem).getProperty("value");
				if(value){
					ret = value;	
				}
				else{
					ret = domainManager.getProxy(elem).getProperty("placeholder");	
				}			
			}
			else{
 ...
 */
function RMoTselectCandidateswithSimilarIdentifier(candidates, identifier, type) {
	if (identifier != null) {
		for (var i = 0; i < candidates.length; i++) {
			var idTrim = jsUtil.trimAndFormat("TString", identifier.parameter.value);
			var valTrim = jsUtil.trimAndFormat(identifier.parameter.type, RMoTgetIdentifiedByValue(candidates[i].candidate, identifier, type));
...
function RMoTfindSimilarElement(uiobject, elemProps) {
	...
	if (typeof uiobject !== 'undefined') {
		...
		try {
			...
			RMoTselectCandidateswithSimilarIdentifier(candidates, uiobject.identifier, uiobject.type);
...
function RMoTRetryExecuteVP (expression, endOfVerification, period) {
	if (jsUtil.isSafari()) 
		var devExpr = JSON.parse(expression); /* expression contains a JSON of a DeviceExpression */
	else
