...
JSUtil.prototype={
		...
		parse:function (jsonString) {//RMoTparse
			...
			if(navigator.userAgent.toLowerCase().includes("safari") || navigator.userAgent.toLowerCase().includes("firefox"))
		...
		*/
		...
		contains:function(str, key) {
			return ((typeof str === 'string') && (str.indexOf) && (str.indexOf(key) !== -1));
		},
		...
		isLightningControlInViewPort: function(element) 
		{
		...
		isLightningControlisOnTop: function(element) 
		{
			const rect = element.getBoundingClientRect();
			var document = element.ownerDocument;
			const elementsUnderPoint = document.elementsFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2);
			const isOnTop = (elementsUnderPoint[0] === element);
			return isOnTop;
		},
	
	isLightningControlVisible: function(element)
	{
		...
		if (this.isLightningDisplayNone(element)) 
		{
		...
		{
			if(this.isLightningControlHidden(element))
			{
				ret.visibility = false;
				ret.reachable = false;
			}
			else if(this.isLightningControlOffsetZero(element))
			{
				ret.visibility = false;
				ret.reachable = false;
			}
			else if (!this.isElementInViewportAndOnTop(element)) {
				ret.visibility = false;
			}
		}
		
		if (!this.isSflElementContainer(element)) 
		{
			if (ret.reachable) 
				ret.visibility = true;
			ret.reachable = true;
	...
	isElementInViewportAndOnTop: function(el)
	{
	  if (!el) return false;
	  ...
	  if (!isVisibleInViewport) return false;
	
	  // Check if the element is on top
	  const elementsAtPoint = document.elementsFromPoint(rect.left + rect.width / 2, rect.top + rect.height / 2);
	  const isTopElement = elementsAtPoint.length > 0 && elementsAtPoint[0] === el;
	
	  return isTopElement;
	},

	isLightningDisplayNone: function(element) 
	{
		var displayStyle = false;
		var mainWindow = jsUtil.getMainWindow();
		if(mainWindow && mainWindow.getComputedStyle)
		{
			var allStyle = mainWindow.getComputedStyle(element);
			if (allStyle != null)
				displayStyle = allStyle.getPropertyValue('display') === 'none';
		}
		return displayStyle;
	},

	isLightningControlHidden: function(element)
	{
		var hiddenStyle = false;
		var mainWindow = jsUtil.getMainWindow();
		if(mainWindow && mainWindow.getComputedStyle)
		{
			var allStyle = mainWindow.getComputedStyle(element);
			if (allStyle != null)
				hiddenStyle = allStyle.getPropertyValue('visibility') === 'hidden';
		}
		return hiddenStyle;
	},

	isLightningControlOffsetZero: function(element) 
	{
		return (element.offsetWidth === 0 || element.offsetHeight === 0);
	},
	
	hasSalesforceGlobals: function() 
	{
 		return typeof window.Sfdc !== 'undefined' || typeof window.$A !== 'undefined';
	},
	
	hasSalesforceMetaTags: function()
	{
		const metas = document.getElementsByTagName('meta');
  		for (let i = 0; i < metas.length; i++) 
  		{
    		if (metas[i].getAttribute('name') === 'salesforce-community') 
    		{
      			return true;
   			}
  		}
	...
	isSalesForceApp: function()
	{
		 return this.isSalesforceSite() || this.hasSalesforceGlobals() || this.hasSalesforceMetaTags();
	},
	
	isSflElementContainer: function(element)
	{
		if(element)
			//below commented line might be modified and used in future
			//return (element.hasChildNodes() || element.children.length > 0 || element.scrollHeight > element.clientHeight || this.isSflElementStyleContainer(element)); // || this.isSflRuntimeGeneratedElement(element.tagName));
			return (element.hasChildNodes() || this.isSflRuntimeGeneratedElement(element.tagName));
		return false;
	},

	isSflElementStyleContainer: function (element)
	{
		const style = window.getComputedStyle(element);
   		return ['block', 'flex', 'grid'].includes(style.display);
	},
	
	isSflRuntimeGeneratedElement: function(elementTagName)
	{
		const elem = ["one-","record_flexipage-","forcegenerated-","flexipage-","runtime_platform_actions-","force-","lst"];
		const matches = elem.some(prefix => elementTagName.toLowerCase().startsWith(prefix));
		return matches;
	},

	getUniqueByNestedKey: function(arr, keyName){
		const seenValues = new Set();  
		const visitedObjs = new WeakSet();

		const findKey = obj => {
		if (obj && typeof obj === 'object') {
			if (visitedObjs.has(obj)) return undefined;
			visitedObjs.add(obj);
		...
		return arr.filter(obj => {
			const keyValue = findKey(obj);
			if (keyValue === undefined || seenValues.has(keyValue)) return false;
...
var WebGuiConstants ={
		...
		EXIST_PROP: "exist",
		DATA_KEY:"data-key",
		DATA_AURA_RENDERED_BY:"data-aura-rendered-by",
		DATA_AURA_CLASS:"data-aura-class",
		DATA_AURA_RENDERING_SERVICE_UID:"data-data-rendering-service-uid"
		}; 
...
JSReflection.prototype.createObject = function(domainObj, element, proxyClassName, proxyName) {
	...
	if (jsUtil.isSafari() || navigator.userAgent.toLowerCase().includes("firefox")) {
        proxyClass = webGuiPlaybackProxyMap[proxyClassName];
    }        
    else {
        proxyClass = eval(proxyClassName);    
    }  
...
var RMOT_DRAGSTART_EVENT = "ondragstart";
var RMOT_DROP_EVENT = "ondrop";
var RMOT_DRAG_LEAVE_EVENT = "ondragleave";
var RMOT_DRAG_EVENT = "ondrag";
var dragElement = null;
var lastElementLeave = null;
var onDragElement = null;
...
HTMLDomain.prototype.getProxy = function(element) {
	...
	if (jsUtil.applySimpleOperator(tagName, '-', 'TContains') && !sflProxyMap[tagName]) {
		tagName = 'custom';
	}
	if (htmlProxyMap[tagName]) {
...
webGuiPlaybackProxyMap.HtmlElementProxy = HtmlElementProxy;
...
HtmlElementProxy.prototype.installMutationObserver = function (eventName) {
	if (!this.element["$mutationObserverInstalled"]) {
		...
		if (sflProxyMap[this.element.tagName.toLowerCase()])
			this.element.addEventListener("scroll", function() { rmotRecorder.cacheScreenshot(); });
		else
...
HtmlElementProxy.prototype.eventHandler = function(event) {
	...
	if (jsUtil.isTrusted(event, eventName) && !event.recorded && !event.corrected) {
		...
		checkedElts = jsUtil.getUniqueByNestedKey(checkedElts, 'UID');
...
HtmlElementProxy.prototype.installEventWrapper = function (eventName, handler) {
	...
	if (this.element[eventName + "$alreadyInstalled"] === true) {
		return;
	}
...
HtmlElementProxy.prototype.getEventsToRegister = function() {
	return [RMOT_DOWN_EVENT, RMOT_TRIGGER_EVENT, RMOT_DBLCLICK_EVENT, RMOT_DRAGSTART_EVENT, RMOT_DROP_EVENT, RMOT_DRAG_LEAVE_EVENT, RMOT_DRAG_EVENT];
};
...
HtmlElementProxy.prototype.handleEvent = function(element, eventName, event) {
	...
	else if (eventName == RMOT_DRAGSTART_EVENT) {
		dragElement = this.element.textContent;
		this.clickRecorded = true;
	}
	else if (eventName == RMOT_DROP_EVENT) {
		this.clickRecorded = true;
	}
	else if (eventName == RMOT_DRAG_LEAVE_EVENT) {
		lastElementLeave = event.target;
		return;
	}
	else if (eventName == RMOT_DRAG_EVENT) {
		onDragElement = event.target;
		return;
	}
...
HtmlElementProxy.prototype.getProperty = function(propName) {
	...
	if (propName && propName.toLowerCase() == WebGuiConstants.TAGNAME_PROP.toLowerCase()) {
		...
		propValue = this.getProperty(WebGuiConstants.EXIST_PROP);
	}
...
HtmlElementProxy.prototype.isContainer =  function() {
	...
	if (this.getProperty('class') == 'mblScrollBarWrapper')
		return false;
	//below commented line might be modified and used in future
	//else if(((typeof this.element.className === 'string') && this.element.className.includes('slds')) || sflProxyMap[this.element.tagName])
	else if(sflProxyMap[this.element.tagName])
	{
		return jsUtil.isSflElementContainer(this.element);
	}
...
HtmlElementProxy.prototype.dispatchPressEnterKeyEvent = function() {
	//some of the code in else part is either deprecated or does not work on latest controls.
	//so added an if part for SF contols.
	if(this.element.classList.length > 0 && this.element.className.includes('slds'))
	{
...
webGuiPlaybackProxyMap.HtmlCustomProxy = HtmlCustomProxy;
/*
 *
 *	Licensed Materials - Property of IBM and/or HCL
 *
...
webGuiPlaybackProxyMap.HtmlEditableProxy = HtmlEditableProxy;
...
webGuiPlaybackProxyMap.HtmlAProxy = HtmlAProxy;
...
webGuiPlaybackProxyMap.HtmlMediaProxy = HtmlMediaProxy;
...
webGuiPlaybackProxyMap.HtmlInput5Proxy = HtmlInput5Proxy;
...
HtmlInput5Proxy.prototype.getEventsToRegister = function() {
	/* 
			 * Registering onmouseUp, onmouseDown, doubleclick onkeydown, and onkeyup events only for inputHTML5list proxy.
			 * For rest HTML5 controls return only oninput. i am doing this only for inputHTML5list control because we are 
			 * not sure if it is required for otherhtml5 controls at the moment.
			 * added by sriharsha.pooma@hcl-software.com
			 * 
			 */
		if( this.proxyName == "inputHTML5list")
			return [RMOT_DOWN_EVENT, RMOT_TRIGGER_EVENT, RMOT_DBLCLICK_EVENT, "onkeydown", "onkeyup"];	
	    return ["oninput"];
};
...
webGuiPlaybackProxyMap.HtmlInput5ColorProxy = HtmlInput5ColorProxy;
...
webGuiPlaybackProxyMap.HtmlInput5DateProxy = HtmlInput5DateProxy;
...
webGuiPlaybackProxyMap.HtmlSelectProxy = HtmlSelectProxy;
...
webGuiPlaybackProxyMap.HtmlOptionProxy = HtmlOptionProxy;
...
webGuiPlaybackProxyMap.HtmlTextInputProxy = HtmlTextInputProxy;
...
HtmlTextInputProxy.prototype._pressEnter = HtmlElementProxy.prototype.pressEnter;
HtmlTextInputProxy.prototype.pressEnter = function()
{
	if (this.element.type === 'password' && this.element.closest('form') !== null && jsUtil.isSalesForceApp()) 
	{
...
webGuiPlaybackProxyMap.HtmlTextSearchInputProxy = HtmlTextSearchInputProxy;
...
webGuiPlaybackProxyMap.HtmlTextAreaProxy = HtmlTextAreaProxy;
...
webGuiPlaybackProxyMap.HtmlInputFileProxy = HtmlInputFileProxy;
...
webGuiPlaybackProxyMap.HtmlCheckBoxRadioProxy = HtmlCheckBoxRadioProxy;
...
HtmlCheckBoxRadioProxy.prototype.getEventsToRegister = function(){
	...
	if (this.element[RMoTclickEvent]
		|| this.element.getAttribute('data-propertyuihint')  || this.element.getAttribute('data-indeterminate')) return [RMOT_TRIGGER_EVENT];
...
webGuiPlaybackProxyMap.HtmlSubmitProxy = HtmlSubmitProxy;
...
HtmlSubmitProxy.prototype._click= HtmlSubmitProxy.prototype.click;
HtmlSubmitProxy.prototype.click = function() {
	//It mostly happens with sales force controls whose aria-disabled is false
	if(this.element.classList.length > 0 && this.element.className.includes('slds') && this._getProperty("aria-disabled") == "false")
	{
		//direct click using out framework is not working on this.
		//so clicking directly on element which we have found.
		this.element.click();	
		return RMOT_SUCCESS;
	}
	var retVal = this._click();
	return retVal;
};
/*
 *
...
webGuiPlaybackProxyMap.HtmlSvgProxy = HtmlSvgProxy;
...
webGuiPlaybackProxyMap.HtmlAreaProxy = HtmlAreaProxy;
...
webGuiPlaybackProxyMap.HtmlBodyProxy = HtmlBodyProxy;
...
webGuiPlaybackProxyMap.HtmlCanvasProxy = HtmlCanvasProxy;
...
webGuiPlaybackProxyMap.HtmlFrameProxy = HtmlFrameProxy;
...
webGuiPlaybackProxyMap.HtmlTableHeaderProxy = HtmlTableHeaderProxy;
...
webGuiPlaybackProxyMap.HtmlTableCellProxy = HtmlTableCellProxy;
...
var sflProxyMap = {
	"lightning-primitive-input-simple": "SFLPrimitiveInputSample",
	"lightning-input": "SFLInput",
	"records-record-layout-base-input": "SFLRecordsRecordLayoutBaseInput",
	"records-record-layout-item": "SFLRecordsRecordLayoutItem",
	"records-record-layout-row": "SFLRecordsRecordLayoutRow",
	"records-record-layout-section": "SFLRecordsRecordLayoutSection",
	"records-record-layout-block": "SFLRecordsRecordLayoutBlock",
	"records-lwc-record-layout": "SFLRecordsLWCRecordLayout",
	"records-base-record-form": "SFLRecordsBaseRecordForm",
	"records-lwc-detail-panel": "SFLRecordsLWCDetailPanel",
	"records-record-layout-event-broker": "SFLRecordsRecordLayoutEventBroker",
	"records-modal-lwc-detail-panel-wrapper": "SFLRecordsModalLWCDetailPanelWrapper",
	"lightning-message-context-consumer": "SFLLightningMessageContextConsumer",
	"lightning-base-combobox": "SFLBaseCombobox",
	"lightning-base-combobox-item": "SFLBaseComboboxItem",
	"lightning-primitive-icon": "SFLPrimitiveIcon",
	"one-app-nav-bar-menu-button": "SFLOneAppNavBarMenuButton",
	"lightning-input-field": "SFLInputField",
	"lightning-combobox": "SFLCombobox",
	"lightning-icon": "SFLIcon",
	"lightning-messages": "SFLMessages",
	"lightning-record-edit-form-create": "SFLRecordEditFormCreate",
	"lightning-record-edit-form": "SFLRecordEditForm",
	"lightning-button": "SFLButton",
	"lightning-textarea": "SFLTextarea",
	"lightning-picklist": "SFLPicklist",
	"one-app-nav-bar-item-dropdown": "SFLOneAppNavBarItemDropdown",
	"one-app-nav-bar-item-root": "SFLOneAppNavBarItemRoot",
	"nav": "SFLNav",
	"one-app-nav-bar": "SFLOneAppNavBar",
	"one-appnav": "SFLOneAppNav",
	"section": "SFLSection",
	"lightning-timepicker": "SFLTimePicker",
	"lightning-button-icon": "SFLButtonIcon",
	"lightning-formatted-rich-text": "SFLFormattedRichText",
	"lightning-base-combobox-formatted-text": "SFLBaseComboboxFormattedText",
	"one-app-nav-bar-menu-item": "SFLOneAppNavBarMenuItem",
	"lightning-datepicker": "SFLDatePicker"
	};
...
SFLDomain.prototype = new HTMLDomain();

webGuiPlaybackProxyMap.SFLDomain = SFLDomain;

SFLDomain.prototype.registerSFLDomain = function()
{
	var domain = this;
	domainManager.registerDomain(domain);
	if (jsUtil.isRecordingMode()) 
	{ // Recording only
		rmotRecorder.log("PARSEDOCUMENT================registerSFLDomain");
...
SFLDomain.prototype.init = function()
{
	//domainManager.registerDomain(this);
	var domain = this;
	if (window.Sfdc || window.sforce || window.visualforce || document.body.classList.contains("slds") || window.$A) 
	{
		if (document.readyState === "complete" || typeof $A !== "undefined" && $A.get) 
		{
			this.registerSFLDomain();
		} 
		else 
		{
			window.addEventListener("load", function() 
			{
				...
				{
					console.log("Can not register SFL domain at this time!");
				}
			});
		}
	}
...
SFLDomain.prototype.getProxy = function(element)
{
	if (element == undefined || element == null)
		return null;
	...
	if (jsUtil.applySimpleOperator(tagName, '-', 'TContains') && !tagName.toLowerCase().startsWith('lightning')) 
	{
		tagName = 'custom';
	}
	if (sflProxyMap[tagName]) 
	{
		proxyClass = sflProxyMap[tagName];
	} 
	else 
	{
		proxyClass = (element.isContentEditable) ? sflProxyMap["editable"] : sflProxyMap["default"];
	}
...
SFLDomain.prototype.getDomainName = function()
{
	return SFL_DOMAIN_NAME;
};
 ...
 */
 ...
 SFLBaseCombobox = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLBaseCombobox.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLBaseCombobox = SFLBaseCombobox;

SFLBaseCombobox.prototype._applyDecoratedProps = SFLBaseCombobox.prototype.applyDecoratedProps;
SFLBaseCombobox.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLBaseCombobox.prototype._isVisible = SFLBaseCombobox.prototype.isVisible;
SFLBaseCombobox.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLBaseComboboxItem = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};


SFLBaseComboboxItem.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLBaseComboboxItem = SFLBaseComboboxItem;

SFLBaseComboboxItem.prototype._applyDecoratedProps = SFLBaseComboboxItem.prototype.applyDecoratedProps;
SFLBaseComboboxItem.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};

SFLBaseComboboxItem.prototype._isVisible = SFLBaseComboboxItem.prototype.isVisible;
SFLBaseComboboxItem.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLButton = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLButton.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLButton = SFLButton;

SFLButton.prototype._applyDecoratedProps = SFLButton.prototype.applyDecoratedProps;
SFLButton.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
	//this.name = this.getProperty("name");
	//this.label = this.getProperty("label");
};
/*
SFLButton.prototype._getProperty = SFLButton.prototype.getProperty;
SFLButton.prototype.getProperty = function(propertyName) {
	var propertyValue = null;
	switch (propertyName) {
		case "name":
			propertyValue = this.element["name"];
			break;
		case "label":
			propertyValue = this.element["label"];
			break;
		default:
			propertyValue = this._getProperty(propertyName);
	}
...
SFLButton.prototype._isVisible = SFLButton.prototype.isVisible;
SFLButton.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
/*
SFLButton.prototype.getPropertiesToSave = function() {
	return ["name", "label"];
};
*/
/*
 ...
 */
 ...
 SFLCombobox = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLCombobox.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLCombobox = SFLCombobox;

SFLCombobox.prototype._applyDecoratedProps = SFLCombobox.prototype.applyDecoratedProps;
SFLCombobox.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLCombobox.prototype._isVisible = SFLCombobox.prototype.isVisible;
SFLCombobox.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLIcon = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
	/*
	const sizeThreshold = 2;
...
SFLIcon.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLIcon = SFLIcon;

SFLIcon.prototype._applyDecoratedProps = SFLIcon.prototype.applyDecoratedProps;
SFLIcon.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
	this.iconName = this.getProperty("iconName");
};

SFLIcon.prototype._getProperty = SFLIcon.prototype.getProperty;
SFLIcon.prototype.getProperty = function(propertyName) {
	var propertyValue = null;
	switch (propertyName) {
		case "iconName":
			propertyValue = this.element["iconName"];
			break;
		default:
			propertyValue = this._getProperty(propertyName);
	}
...
SFLIcon.prototype._isVisible = SFLIcon.prototype.isVisible;
SFLIcon.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	//ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};

SFLIcon.prototype.getPropertiesToSave = function() {
	return ["iconName"];
};

SFLIcon.prototype._click = SFLIcon.prototype.click;
SFLIcon.prototype.click = function() {
	var retVal = RMOT_FAILURE;
	try
	{
		this.element.focus();
		this.element.click();
 ...
 */
 ...
 SFLInput = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLInput.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLInput = SFLInput;

SFLInput.prototype._applyDecoratedProps = SFLInput.prototype.applyDecoratedProps;
SFLInput.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLInput.prototype._isVisible = SFLInput.prototype.isVisible;
SFLInput.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLInputField = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLInputField.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLInputField = SFLInputField;

SFLInputField.prototype._applyDecoratedProps = SFLInputField.prototype.applyDecoratedProps;
SFLInputField.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLInputField.prototype._isVisible = SFLInputField.prototype.isVisible;
SFLInputField.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLLightningMessageContextConsumer = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLLightningMessageContextConsumer.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLLightningMessageContextConsumer = SFLLightningMessageContextConsumer;

SFLLightningMessageContextConsumer.prototype._applyDecoratedProps = SFLLightningMessageContextConsumer.prototype.applyDecoratedProps;
SFLLightningMessageContextConsumer.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLLightningMessageContextConsumer.prototype._isVisible = SFLLightningMessageContextConsumer.prototype.isVisible;
SFLLightningMessageContextConsumer.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLMessages = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLMessages.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLMessages = SFLMessages;

SFLMessages.prototype._applyDecoratedProps = SFLMessages.prototype.applyDecoratedProps;
SFLMessages.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLMessages.prototype._isVisible = SFLMessages.prototype.isVisible;
SFLMessages.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLOneAppNavBarMenuButton = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLOneAppNavBarMenuButton.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLOneAppNavBarMenuButton = SFLOneAppNavBarMenuButton;

SFLOneAppNavBarMenuButton.prototype._applyDecoratedProps = SFLOneAppNavBarMenuButton.prototype.applyDecoratedProps;
SFLOneAppNavBarMenuButton.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLOneAppNavBarMenuButton.prototype._isVisible = SFLOneAppNavBarMenuButton.prototype.isVisible;
SFLOneAppNavBarMenuButton.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLPicklist = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLPicklist.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLPicklist = SFLPicklist;

SFLPicklist.prototype._applyDecoratedProps = SFLPicklist.prototype.applyDecoratedProps;
SFLPicklist.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLPicklist.prototype._isVisible = SFLPicklist.prototype.isVisible;
SFLPicklist.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLPrimitiveIcon = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
	/*
	const sizeThreshold = 2;
...
SFLPrimitiveIcon.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLPrimitiveIcon = SFLPrimitiveIcon;

SFLPrimitiveIcon.prototype._applyDecoratedProps = SFLPrimitiveIcon.prototype.applyDecoratedProps;
SFLPrimitiveIcon.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
	this.iconName = this.getProperty("iconName");
};
SFLPrimitiveIcon.prototype._isVisible = SFLPrimitiveIcon.prototype.isVisible;
SFLPrimitiveIcon.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	//ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};

SFLPrimitiveIcon.prototype._getProperty = SFLPrimitiveIcon.prototype.getProperty;
SFLPrimitiveIcon.prototype.getProperty = function(propertyName) {
	var propertyValue = null;
	switch (propertyName) {
		case "iconName":
			propertyValue = this.element["iconName"];
...
SFLPrimitiveIcon.prototype.getPropertiesToSave = function() {
	return ["iconName"];
};

SFLPrimitiveIcon.prototype._click = SFLPrimitiveIcon.prototype.click;
SFLPrimitiveIcon.prototype.click = function() {
	var retVal = RMOT_FAILURE;
	try
	{
		this.element.focus();
 ...
 */
 ...
 SFLPrimitiveInputSample = function(domainObj, element)
 {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLPrimitiveInputSample.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLPrimitiveInputSample = SFLPrimitiveInputSample;

SFLPrimitiveInputSample.prototype._applyDecoratedProps = SFLPrimitiveInputSample.prototype.applyDecoratedProps;
SFLPrimitiveInputSample.prototype.applyDecoratedProps = function(targetElement) 
{
	this._applyDecoratedProps(targetElement);
};
SFLPrimitiveInputSample.prototype._isVisible = SFLPrimitiveInputSample.prototype.isVisible;
SFLPrimitiveInputSample.prototype.isVisible = function() 
{
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
 ...
 */
 ...
 SFLRecordEditForm = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordEditForm.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordEditForm = SFLRecordEditForm;

SFLRecordEditForm.prototype._applyDecoratedProps = SFLRecordEditForm.prototype.applyDecoratedProps;
SFLRecordEditForm.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordEditForm.prototype._isVisible = SFLRecordEditForm.prototype.isVisible;
SFLRecordEditForm.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordEditFormCreate = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordEditFormCreate.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordEditFormCreate = SFLRecordEditFormCreate;

SFLRecordEditFormCreate.prototype._applyDecoratedProps = SFLRecordEditFormCreate.prototype.applyDecoratedProps;
SFLRecordEditFormCreate.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordEditFormCreate.prototype._isVisible = SFLRecordEditFormCreate.prototype.isVisible;
SFLRecordEditFormCreate.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsBaseRecordForm = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsBaseRecordForm.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsBaseRecordForm = SFLRecordsBaseRecordForm;

SFLRecordsBaseRecordForm.prototype._applyDecoratedProps = SFLRecordsBaseRecordForm.prototype.applyDecoratedProps;
SFLRecordsBaseRecordForm.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsBaseRecordForm.prototype._isVisible = SFLRecordsBaseRecordForm.prototype.isVisible;
SFLRecordsBaseRecordForm.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsLWCDetailPanel = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsLWCDetailPanel.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsLWCDetailPanel = SFLRecordsLWCDetailPanel;

SFLRecordsLWCDetailPanel.prototype._applyDecoratedProps = SFLRecordsLWCDetailPanel.prototype.applyDecoratedProps;
SFLRecordsLWCDetailPanel.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsLWCDetailPanel.prototype._isVisible = SFLRecordsLWCDetailPanel.prototype.isVisible;
SFLRecordsLWCDetailPanel.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsLWCRecordLayout = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsLWCRecordLayout.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsLWCRecordLayout = SFLRecordsLWCRecordLayout;

SFLRecordsLWCRecordLayout.prototype._applyDecoratedProps = SFLRecordsLWCRecordLayout.prototype.applyDecoratedProps;
SFLRecordsLWCRecordLayout.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsLWCRecordLayout.prototype._isVisible = SFLRecordsLWCRecordLayout.prototype.isVisible;
SFLRecordsLWCRecordLayout.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsModalLWCDetailPanelWrapper = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsModalLWCDetailPanelWrapper.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsModalLWCDetailPanelWrapper = SFLRecordsModalLWCDetailPanelWrapper;

SFLRecordsModalLWCDetailPanelWrapper.prototype._applyDecoratedProps = SFLRecordsModalLWCDetailPanelWrapper.prototype.applyDecoratedProps;
SFLRecordsModalLWCDetailPanelWrapper.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsModalLWCDetailPanelWrapper.prototype._isVisible = SFLRecordsModalLWCDetailPanelWrapper.prototype.isVisible;
SFLRecordsModalLWCDetailPanelWrapper.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsRecordLayoutBaseInput = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsRecordLayoutBaseInput.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsRecordLayoutBaseInput = SFLRecordsRecordLayoutBaseInput;

SFLRecordsRecordLayoutBaseInput.prototype._applyDecoratedProps = SFLRecordsRecordLayoutBaseInput.prototype.applyDecoratedProps;
SFLRecordsRecordLayoutBaseInput.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsRecordLayoutBaseInput.prototype._isVisible = SFLRecordsRecordLayoutBaseInput.prototype.isVisible;
SFLRecordsRecordLayoutBaseInput.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsRecordLayoutBlock = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsRecordLayoutBlock.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsRecordLayoutBlock = SFLRecordsRecordLayoutBlock;

SFLRecordsRecordLayoutBlock.prototype._applyDecoratedProps = SFLRecordsRecordLayoutBlock.prototype.applyDecoratedProps;
SFLRecordsRecordLayoutBlock.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsRecordLayoutBlock.prototype._isVisible = SFLRecordsRecordLayoutBlock.prototype.isVisible;
SFLRecordsRecordLayoutBlock.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsRecordLayoutEventBroker = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsRecordLayoutEventBroker.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsRecordLayoutEventBroker = SFLRecordsRecordLayoutEventBroker;

SFLRecordsRecordLayoutEventBroker.prototype._applyDecoratedProps = SFLRecordsRecordLayoutEventBroker.prototype.applyDecoratedProps;
SFLRecordsRecordLayoutEventBroker.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsRecordLayoutEventBroker.prototype._isVisible = SFLRecordsRecordLayoutEventBroker.prototype.isVisible;
SFLRecordsRecordLayoutEventBroker.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsRecordLayoutItem = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsRecordLayoutItem.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsRecordLayoutItem = SFLRecordsRecordLayoutItem;

SFLRecordsRecordLayoutItem.prototype._applyDecoratedProps = SFLRecordsRecordLayoutItem.prototype.applyDecoratedProps;
SFLRecordsRecordLayoutItem.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsRecordLayoutItem.prototype._isVisible = SFLRecordsRecordLayoutItem.prototype.isVisible;
SFLRecordsRecordLayoutItem.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsRecordLayoutRow = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsRecordLayoutRow.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsRecordLayoutRow = SFLRecordsRecordLayoutRow;
SFLRecordsRecordLayoutRow.prototype._applyDecoratedProps = SFLRecordsRecordLayoutRow.prototype.applyDecoratedProps;
SFLRecordsRecordLayoutRow.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsRecordLayoutRow.prototype._isVisible = SFLRecordsRecordLayoutRow.prototype.isVisible;
SFLRecordsRecordLayoutRow.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLRecordsRecordLayoutSection = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLRecordsRecordLayoutSection.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLRecordsRecordLayoutSection = SFLRecordsRecordLayoutSection;

SFLRecordsRecordLayoutSection.prototype._applyDecoratedProps = SFLRecordsRecordLayoutSection.prototype.applyDecoratedProps;
SFLRecordsRecordLayoutSection.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLRecordsRecordLayoutSection.prototype._isVisible = SFLRecordsRecordLayoutSection.prototype.isVisible;
SFLRecordsRecordLayoutSection.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLTextarea = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLTextarea.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLTextarea = SFLTextarea;

SFLTextarea.prototype._applyDecoratedProps = SFLTextarea.prototype.applyDecoratedProps;
SFLTextarea.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
	this.label = this.getProperty("label");
};

SFLTextarea.prototype._getProperty = SFLTextarea.prototype.getProperty;
SFLTextarea.prototype.getProperty = function(propertyName) {
	var propertyValue = null;
	switch (propertyName) {
		case "label":
			propertyValue = this.element["label"];
...
SFLTextarea.prototype._isVisible = SFLTextarea.prototype.isVisible;
SFLTextarea.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLOneAppNavBarItemDropdown = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLOneAppNavBarItemDropdown.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLOneAppNavBarItemDropdown = SFLOneAppNavBarItemDropdown;

SFLOneAppNavBarItemDropdown.prototype._applyDecoratedProps = SFLOneAppNavBarItemDropdown.prototype.applyDecoratedProps;
SFLOneAppNavBarItemDropdown.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLOneAppNavBarItemDropdown.prototype._isVisible = SFLOneAppNavBarItemDropdown.prototype.isVisible;
SFLOneAppNavBarItemDropdown.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLOneAppNavBarItemRoot = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLOneAppNavBarItemRoot.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLOneAppNavBarItemRoot = SFLOneAppNavBarItemRoot;

SFLOneAppNavBarItemRoot.prototype._applyDecoratedProps = SFLOneAppNavBarItemRoot.prototype.applyDecoratedProps;
SFLOneAppNavBarItemRoot.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLOneAppNavBarItemRoot.prototype._isVisible = SFLOneAppNavBarItemRoot.prototype.isVisible;
SFLOneAppNavBarItemRoot.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLNav = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLNav.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLNav = SFLNav;

SFLNav.prototype._applyDecoratedProps = SFLNav.prototype.applyDecoratedProps;
SFLNav.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLNav.prototype._isVisible = SFLNav.prototype.isVisible;
SFLNav.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLOneAppNavBar = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLOneAppNavBar.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLOneAppNavBar = SFLOneAppNavBar;

SFLOneAppNavBar.prototype._applyDecoratedProps = SFLOneAppNavBar.prototype.applyDecoratedProps;
SFLOneAppNavBar.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLOneAppNavBar.prototype._isVisible = SFLOneAppNavBar.prototype.isVisible;
SFLOneAppNavBar.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLOneAppNav = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLOneAppNav.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLOneAppNav = SFLOneAppNav;

SFLOneAppNav.prototype._applyDecoratedProps = SFLOneAppNav.prototype.applyDecoratedProps;
SFLOneAppNav.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLOneAppNav.prototype._isVisible = SFLOneAppNav.prototype.isVisible;
SFLOneAppNav.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
 ...
 */
 ...
 SFLSection = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLSection.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLSection = SFLSection;

SFLSection.prototype._applyDecoratedProps = SFLSection.prototype.applyDecoratedProps;
SFLSection.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLSection.prototype._isVisible = SFLSection.prototype.isVisible;
SFLSection.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
...
SFLTimePicker = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};
SFLTimePicker.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLTimePicker = SFLTimePicker;

SFLTimePicker.prototype._applyDecoratedProps = SFLTimePicker.prototype.applyDecoratedProps;
SFLTimePicker.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLTimePicker.prototype._isVisible = SFLTimePicker.prototype.isVisible;
SFLTimePicker.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
...
SFLButtonIcon = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLButtonIcon.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLButtonIcon = SFLButtonIcon;

SFLButtonIcon.prototype._applyDecoratedProps = SFLButtonIcon.prototype.applyDecoratedProps;
SFLButtonIcon.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
	this.iconName = this.getProperty("iconName");
	this.name = this.getProperty("name");
	this.title = this.getProperty("title");
};
SFLButtonIcon.prototype._isVisible = SFLButtonIcon.prototype.isVisible;
SFLButtonIcon.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};

SFLButtonIcon.prototype._getProperty = SFLButtonIcon.prototype.getProperty;
SFLButtonIcon.prototype.getProperty = function(propertyName) {
	var propertyValue = null;
	switch (propertyName) {
		case "iconName":
			propertyValue = this.element["iconName"];
		...
		case "name":
			...
			break;
		case "title":
			propertyValue = this.element["title"];
			break;
		default:
			propertyValue = this._getProperty(propertyName);
	}
...
SFLButtonIcon.prototype.getPropertiesToSave = function() {
	return ["iconName", "name", "title"];
};/*
 *
 *	Licensed Materials - Property of IBM
 *
 ...
 */
 ...
 SFLFormattedRichText = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLFormattedRichText.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLFormattedRichText = SFLFormattedRichText;

SFLFormattedRichText.prototype._applyDecoratedProps = SFLFormattedRichText.prototype.applyDecoratedProps;
SFLFormattedRichText.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
	targetElement.value = this.getProperty("value");
};

SFLFormattedRichText.prototype._isVisible = SFLFormattedRichText.prototype.isVisible;
SFLFormattedRichText.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);
	return ret;
};

SFLFormattedRichText.prototype._getProperty = SFLFormattedRichText.prototype.getProperty;
SFLFormattedRichText.prototype.getProperty = function(propName) {
	var propValue = "";
	if (propName == WebGuiConstants.CONTENT_PROP) 
	{
		// intentionally blank, do not return anything
...
SFLFormattedRichText.prototype.getPropertiesToSave = function() 
{
	return ["value"];
};

SFLFormattedRichText.prototype._click= SFLFormattedRichText.prototype.click;
SFLFormattedRichText.prototype.click = function() 
{
	var retVal = RMOT_FAILURE;
	try
	{
		//TODO : There is no event listener attached to this control so direct click will work only.
		//In future we need to check how to find out if there is any listener attached
 ...
 */
 ...
 SFLBaseComboboxFormattedText = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLBaseComboboxFormattedText.prototype = new HtmlElementProxy();

webGuiPlaybackProxyMap.SFLBaseComboboxFormattedText = SFLBaseComboboxFormattedText;

SFLBaseComboboxFormattedText.prototype._applyDecoratedProps = SFLBaseComboboxFormattedText.prototype.applyDecoratedProps;
SFLBaseComboboxFormattedText.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};

SFLBaseComboboxFormattedText.prototype._isVisible = SFLBaseComboboxFormattedText.prototype.isVisible;
SFLBaseComboboxFormattedText.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);
	return ret;
};/*
...
//	one-app-nav-bar-menu-item	: SFLOneAppNavBarMenuItem
 SFLOneAppNavBarMenuItem = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLOneAppNavBarMenuItem.prototype = new HtmlElementProxy();
webGuiPlaybackProxyMap.SFLOneAppNavBarMenuItem = SFLOneAppNavBarMenuItem;

SFLOneAppNavBarMenuItem.prototype._applyDecoratedProps = SFLOneAppNavBarMenuItem.prototype.applyDecoratedProps;
SFLOneAppNavBarMenuItem.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLOneAppNavBarMenuItem.prototype._isVisible = SFLOneAppNavBarMenuItem.prototype.isVisible;
SFLOneAppNavBarMenuItem.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};
...
//	lightning-datepicker	:	SFLDatePicker
SFLDatePicker = function(domainObj, element) {
	if (arguments.length == 0) return; // don't do anything
	HtmlElementProxy.apply(this, arguments);
};

SFLDatePicker.prototype = new HtmlElementProxy();
webGuiPlaybackProxyMap.SFLDatePicker = SFLDatePicker;

SFLDatePicker.prototype._applyDecoratedProps = SFLDatePicker.prototype.applyDecoratedProps;
SFLDatePicker.prototype.applyDecoratedProps = function(targetElement) {
	this._applyDecoratedProps(targetElement);
};
SFLDatePicker.prototype._isVisible = SFLDatePicker.prototype.isVisible;
SFLDatePicker.prototype.isVisible = function() {
	var ret = { visibility: true, propagation: false, reachable: true };
	ret = jsUtil.isLightningControlVisible(this.element);	
	return ret;
};/*
...
webGuiPlaybackProxyMap.DojoWidgetProxy = DojoWidgetProxy;
...
webGuiPlaybackProxyMap.DojoElementProxy = DojoElementProxy;
...
webGuiPlaybackProxyMap.DojoMobileWidgetProxy = DojoMobileWidgetProxy;
...
webGuiPlaybackProxyMap.DojoMobileTextBoxProxy = DojoMobileTextBoxProxy;
...
webGuiPlaybackProxyMap.DojoMobileSearchBoxProxy = DojoMobileSearchBoxProxy;
...
webGuiPlaybackProxyMap.DojoMobileTextAreaProxy = DojoMobileTextAreaProxy;
...
webGuiPlaybackProxyMap.DojoMobileCheckBoxRadioProxy = DojoMobileCheckBoxRadioProxy;
...
webGuiPlaybackProxyMap.DojoMobileToggleButtonProxy = DojoMobileToggleButtonProxy;
...
webGuiPlaybackProxyMap.DojoMobileSwitchProxy = DojoMobileSwitchProxy;
...
webGuiPlaybackProxyMap.DojoMobileSliderProxy = DojoMobileSliderProxy;
...
webGuiPlaybackProxyMap.DojoMobileSwapviewProxy = DojoMobileSwapviewProxy;
...
webGuiPlaybackProxyMap.DojoMobileAccordionTitleProxy = DojoMobileAccordionTitleProxy;
...
webGuiPlaybackProxyMap.DojoMobileMediaProxy = DojoMobileMediaProxy;
...
webGuiPlaybackProxyMap.DojoMobileSpinWheelSlotProxy = DojoMobileSpinWheelSlotProxy;
...
webGuiPlaybackProxyMap.DojoMobileSpinWheelPickerProxy = DojoMobileSpinWheelPickerProxy;
...
webGuiPlaybackProxyMap.DojoMobileValuePickerProxy = DojoMobileValuePickerProxy;
...
webGuiPlaybackProxyMap.DojoMobileValuePickerSlotProxy = DojoMobileValuePickerSlotProxy;
...
webGuiPlaybackProxyMap.DojoMobileGaugeProxy = DojoMobileGaugeProxy;
...
webGuiPlaybackProxyMap.DojoMobileListItemProxy = DojoMobileListItemProxy;
...
webGuiPlaybackProxyMap.DojoMobileScrollableviewProxy = DojoMobileScrollableviewProxy;
...
webGuiPlaybackProxyMap.JQueryUIElementProxy = JQueryUIElementProxy;
...
webGuiPlaybackProxyMap.JQueryUIWidgetProxy = JQueryUIWidgetProxy;
...
webGuiPlaybackProxyMap.JQueryMobileElementProxy = JQueryMobileElementProxy;
...
webGuiPlaybackProxyMap.JQueryMobileWidgetProxy = JQueryMobileWidgetProxy;
...
webGuiPlaybackProxyMap.JQueryMobileButtonProxy = JQueryMobileButtonProxy;
...
webGuiPlaybackProxyMap.JQueryMobileListviewItemProxy = JQueryMobileListviewItemProxy;
...
webGuiPlaybackProxyMap.JQueryMobileTextInputProxy = JQueryMobileTextInputProxy;
...
webGuiPlaybackProxyMap.JQueryMobileTextAreaProxy = JQueryMobileTextAreaProxy;
...
webGuiPlaybackProxyMap.JQueryMobileSliderProxy = JQueryMobileSliderProxy;
...
webGuiPlaybackProxyMap.JQueryMobileRangeSliderProxy = JQueryMobileRangeSliderProxy;
...
webGuiPlaybackProxyMap.JQueryMobileSelectProxy = JQueryMobileSelectProxy;
...
webGuiPlaybackProxyMap.JQueryCheckBoxRadioProxy = JQueryCheckBoxRadioProxy;
...
webGuiPlaybackProxyMap.JQueryMobileFlipSwitchProxy = JQueryMobileFlipSwitchProxy;
...
webGuiPlaybackProxyMap.JQueryMobile_13x_FlipSwitchProxy = JQueryMobile_13x_FlipSwitchProxy;
...
webGuiPlaybackProxyMap.JQueryMobileCollapsibleProxy = JQueryMobileCollapsibleProxy;
...
webGuiPlaybackProxyMap.JQueryMobileCollapsibleContentProxy = JQueryMobileCollapsibleContentProxy;
...
webGuiPlaybackProxyMap.JQueryMobileDatePickerProxy = JQueryMobileDatePickerProxy;
...
webGuiPlaybackProxyMap.JQueryMobileControlGroupProxy = JQueryMobileControlGroupProxy;
...
webGuiPlaybackProxyMap.JQueryUIButtonProxy = JQueryUIButtonProxy;
...
webGuiPlaybackProxyMap.JQueryUITabProxy = JQueryUITabProxy;
...
webGuiPlaybackProxyMap.JQueryUIDateInputProxy = JQueryUIDateInputProxy;
...
webGuiPlaybackProxyMap.JQueryProgressBarProxy = JQueryProgressBarProxy;
...
webGuiPlaybackProxyMap.JQueryUIDateProxy = JQueryUIDateProxy;
...
webGuiPlaybackProxyMap.JQueryUITextInputProxy = JQueryUITextInputProxy;
...
webGuiPlaybackProxyMap.JQueryUIaccordionHeaderProxy = JQueryUIaccordionHeaderProxy;
...
webGuiPlaybackProxyMap.JQueryUISliderProxy = JQueryUISliderProxy;
...
webGuiPlaybackProxyMap.JQueryUISpinnerProxy = JQueryUISpinnerProxy;
...
webGuiPlaybackProxyMap.JQueryUIDialogProxy = JQueryUIDialogProxy;
...
webGuiPlaybackProxyMap.JQueryUIMenuItemProxy = JQueryUIMenuItemProxy;
...
webGuiPlaybackProxyMap.JQueryUIMenuProxy = JQueryUIMenuProxy;
...
webGuiPlaybackProxyMap.JQueryUISelectmenuProxy = JQueryUISelectmenuProxy;
...
webGuiPlaybackProxyMap.JQueryUIControlGroupProxy = JQueryUIControlGroupProxy;
...
WebGuiRecorderInterface.prototype ={
		...
		recordEvent:function(event, eventName, tagName, element, parameters){
			...
			var locator = null;
			if(eventName == RMOT_DROP_EVENT){
				const gridElement = document.getElementById('grid');
				let isGridLayout = false;
				let isDivLocator = false;
				if (gridElement) {
					const style = getComputedStyle(document.getElementById('grid'));
					isGridLayout = style.display === 'grid' || style.display === 'inline-grid';
				}
	
				if(!isGridLayout){	
					if (dragElement === element.textContent) {
					if (!element.previousElementSibling && !element.nextElementSibling) {
						element = lastElementLeave;
					} else {
						element = element.previousElementSibling || element.nextElementSibling || element;
					}					
					isDivLocator= true;
					} else if(Array.isArray(element.RMoTChildren)&& element.RMoTChildren.length > 0){
						element = lastElementLeave.previousElementSibling || lastElementLeave.nextElementSibling || lastElementLeave;			
					}
					const listElement = event.target.closest('ul, ol');
					const divElement = event.target.closest('div');
					if (listElement || (divElement && isDivLocator)) {
						const orientation = this.getListOrientation(listElement);
						locator = this.getPointerPositionRelativeToElement(event, element, orientation);
					}
				}else{
					element = onDragElement;
				}
			}
		...
		getListOrientation: function(listElement) {
			if(listElement == null){
			return 'vertical';
			}
			...
			const dx = Math.abs(secondItem.left - firstItem.left);
			const dy = Math.abs(secondItem.top - firstItem.top);
	    return dx > dy ? 'horizontal' : 'vertical';
		},
		
	    getPointerPositionRelativeToElement: function(e, element, orientation) {
	    const rect = element.getBoundingClientRect();
	
			if (orientation === 'vertical') {
				const mouseY = e.clientY;
				const midpointY = rect.top + rect.height / 2;
				return mouseY < midpointY ? 'ABOVE' : 'BELOW';
			} else if (orientation === 'horizontal') {
				const mouseX = e.clientX;
				const midpointX = rect.left + rect.width / 2;
				return mouseX < midpointX ? 'LEFT_OF' : 'RIGHT_OF';
			}
		}
};
...
WebGuiRecorder.prototype ={
		...
		visit:function (node, func) {//RMoTvisit	
			...
			if (node && (node.nodeType == window.Node.ELEMENT_NODE || node.nodeType == window.Node.DOCUMENT_NODE)) {
				...
				if (this.isShadowDomControls(node.tagName)) {
				...
				if (node.hasChildNodes() || ((node.shadowRoot != null) && (node.shadowRoot.hasChildNodes()))) {
					children = (node.shadowRoot != null) ? node.shadowRoot.childNodes : node.childNodes;
				}
				else if (assignedNodes != null && assignedNodes.length > 0) {
		...
		buildJsonString:function(eventName, tagName, element, hierarchy, parameters, actionId, locator) {
			var jsonString = null;
			if(jsUtil.isDesktop()){
				jsonString = this.buildJsonStringDesktop(true, eventName, tagName, element, hierarchy, parameters, actionId, null, null, null, null, locator);
			}
			else{ // Mobile
				jsonString = this.buildJsonStringMobile(eventName, tagName, element, hierarchy, parameters, locator);
			}
			return jsonString;	
		},
		
		buildJsonStringMobile:function(eventName, tagName, element, hierarchy, parameters, locator) {//RMoTbuildJsonString
			var proxy = domainManager.getProxy(element);
		...
		isShadowDomControls: function (elementTag) {
			if (elementTag != null && elementTag != undefined && shadowDomControls.indexOf(elementTag.toLowerCase()) >= 0)
				return true;
			return false;
		}
};
